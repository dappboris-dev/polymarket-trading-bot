...
class AutoTradingBot {
    ...
    constructor() {
        ...
        if (!privateKey || privateKey.length < 64) {
            console.error('PRIVATE_KEY not found or invalid in .env');
    ...
    async start() {
        ...
        console.log('\nBalance check (min $500 for trading):');
        ...
        console.log('\nBot started.');
        console.log('Automatic trading active.\n');
    ...
    private startMonitoring() {
        ...
        setInterval(async () => {
            ...
            if (now - this.lastBalanceCheck >= this.balanceCheckInterval) {
                ...
                if (!check.sufficient) {
                    console.log('WARNING: Low balance. Trading requires at least $500 USDC.');
                    check.warnings.forEach(w => console.log(`  ${w}`));
                    console.log('Bot will continue monitoring; trades may be skipped until balance is sufficient.');
                }
    ...
    private async executeTrade(opportunity: TradeOpportunity) {
        ...
        try {
            ...
            console.log('Waiting 2s for position to settle...');
            ...
            console.log('='.repeat(60));
            console.log(`Next trade available in ${this.tradeCooldown / 1000}s\n`);
