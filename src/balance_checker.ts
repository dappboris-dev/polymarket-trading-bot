...
export class BalanceChecker {
    ...
    checkSufficientBalance(balances: BalanceInfo, requiredUsdc: number = 500.0, requiredMatic: number = 0.05): { 
        ...
        if (balances.usdc < requiredUsdc) {
            warnings.push(`Insufficient USDC: $${balances.usdc.toFixed(2)} (min $${requiredUsdc.toFixed(2)})`);
            sufficient = false;
        } else {
        ...
        if (balances.matic < requiredMatic) {
            warnings.push(`Insufficient MATIC: ${balances.matic.toFixed(4)} (min ${requiredMatic.toFixed(4)} for gas)`);
            sufficient = false;
        } else {
